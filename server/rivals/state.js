...
export const state = {
  ...
  queues: { beginner: [], duo: [], squad: [], wave: [] }, // arrays of player ids
  waveMatchId: null,                          // the one shared Wave Survival round
...
export function publicPlayer(p) {
  return { id: p.id, name: p.name, avatar: p.avatar, pos: p.pos, ry: p.ry, anim: p.anim, weapon: p.weapon || 'ar', skins: p.skins || null, platform: p.platform || null };
}
...
export function publicFighter(f) {
  return {
    ...
    hp: Math.round(f.hp), weapon: f.weapon, dead: !!f.dead, skins: f.skins || null, platform: f.platform || null,
  };
}
