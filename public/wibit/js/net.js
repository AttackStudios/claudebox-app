// WebSocket client + interpolation buffer for Wibit Simulator (same pattern as
// the other ClaudeBox games, pointed at /wibit-ws).

const SEND_RATE = 1000 / 14;
...
export class Net {
  ...
  connect() {
    ...
    this.ws = new WebSocket(`${proto}://${location.host}/wibit-ws`);
...
export class InterpBuffer {
  ...
  push(values) {
    ...
    if (this.frames.length > 24) this.frames.splice(0, this.frames.length - 24);
  }
  sample(angles = []) {
